                ...
                void *aux UNUSED)
{
	...
	return ta->timer_expiry < tb->timer_expiry;
}
...
/*
 * Disable interrupts, calculate ending time and add to timer queue
 * that will be monitored by the system timer (timer_interrupt()), block
 * the thread and wait for the system to unblock it. -GYU
 */
void
timer_sleep (int64_t ticks) 
{
  ...
  thread_current()->timer_expiry = end;
  list_insert_ordered(&jeepers_sleepers, &thread_current()->elem, expires_earlier, NULL);
  thread_block();
...
/*
 * Check if there are any threads to wake up (and wake them up == send them
 * to the ready queue). Note that jeepers_sleepers is already sorted.
 */
static void
